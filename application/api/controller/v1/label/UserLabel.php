...
class UserLabel extends Api
{
    ...
    public function labellist()
    {
        ...
        $count = db('label') -> alias('L') -> where($this->_getWhere($params))->whereor(function($query){
            $query->where([['L.is_del','=',0],['L.type','>',2]]);
        })-> join($this ->_getJoin($params)) -> count();
        $data = db('label') -> alias('L') -> where($this->_getWhere($params))->whereor(function($query){
            $query->where([['L.is_del','=',0],['L.type','>',2]]);
        }) -> field($this->_getField($params)) -> join($this ->_getJoin($params))->order('type desc')->limit(($page-1)*$size,$size) -> select();
        if($data){
            foreach($data as $key=>$value){
                if($value['type'] == 2){
                    $data[$key]['type'] = ['type'=>$value['type'],'text'=>lang('自定义标签')];
                }else{
                    $data[$key]['type'] = ['type'=>$value['type'],'text'=>lang('系统标签')];
                    $data[$key]['name'] = lang($data[$key]['name']);
                }
            }
        }
    ...
    public function del(){
        ...
        $result = db('label') -> where($where) -> find();
        if(isset($result['type']) && $result['type'] == 3){
            $this->_returnMsg(['code' => 1, 'msg' => '系统标签，不能删除']);die;
        }
        $data['is_del'] = 1;
    ...
    function _getData(){
        ...
        if($pkId){
            ...
            $data['label_id'] = $pkId;
            $data['name'] = $name;
            $data['type'] = $params && isset($params['type']) ? intval($params['type']) : 2;
