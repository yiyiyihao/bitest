...
class GoodsLabel extends Api
{
    ...
    public function add() {
        ...
        if($pkId){
            $this->_returnMsg(['code' => 0, 'msg' => '成功', 'data' => ['label_id' => $pkId]]);die;
        }else{
    ...
    public function edit() {
        ...
        if($pkId){
            ...
            $data = $this->_getData();
            $rs = db('label')->where(['label_id' => $pkId,'is_del'=>0,'developer_id'=>$params['developer_id']])->update($data);
            if($rs){
                $this->_returnMsg(['code' => 0, 'msg' => '修改成功','data' => ['label_id' => $pkId]]);die;
                //$this->success($msg, url("index", $routes), TRUE);
            }else{
                $this->_returnMsg(['code' => 1, 'msg' => '标签不存在,修改失败']);die;
                //$this->error($msg);
            }
    ...
    function _getData(){
        ...
        if($exist){
            $this->_returnMsg(['code' => 1, 'msg' => '当前标签名称已存在']);die;
        }
        ...
        if($pkId){
            ...
            $data['label_id'] = $pkId;
            $data['name'] = $name;
            $data['type'] = $params && isset($params['type']) ? intval($params['type']) : 1;
