...
class User
{
    ...
    public function assetChange($userId = '', $field = 'balance', $num = 0, $action = '', $msg = '', $extra = [])
    {
        ...
        if (!$user){
            $this->error = lang('用户不存在或已删除');
            return FALSE;
        }
        if (!isset($user[$field])) {
            $this->error = lang('数据库字段不存在');
            return FALSE;
        }
        $fields = ['balance'];
     ...
     */
    public function register($username = '', $password = '', $extra = [],$tag=0,$yz_pwd=0,$token ='',$openid = '')
    {
        if (!$username) {
            $this->error = lang('登录用户名不能为空');
            return FALSE;
        }
        if ($password !== FALSE && !$password && !$yz_pwd) {
            $this->error = lang('登录密码不能为空');
            return FALSE;
        }
    ...
    public function _checkFormat($extra = [])
    {
        ...
        if ($username) {
            ...
            if (!$uncheckName && !preg_match($pattern, $username)) {
                $this->error = lang('登录用户名格式:5-16位字符长度,只能由英文数字下划线组成');
                return FALSE;
            }
            //检查登录用户名是否存在
            $exist = $this->_checkUsername($username);
            if ($exist) {
                $this->error = lang('登录用户名或手机号已经存在');
                return FALSE;
            }
        }
        //检查密码格式
        ...
        if ($password && !preg_match($pattern, $password)) {
            $this->error = lang('登录密码格式:以字母开头，长度在6~20之间，只能包含字母、数字和下划线和@');
            return FALSE;
        }
        $pattern = '/^\d{11}$/';
        if ($phone && !preg_match($pattern, $phone)) {
